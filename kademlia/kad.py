...
class DHTRequestHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        if message_type == "find_node":
            print("目前的鄰近節點有: %s" % self.server.dht.peers())
    ...
    def handle_ping(self, message):
        ...
        peer.pong(socket=self.server.socket,
...
class DHT(object):
    ...
    def iterative_find_nodes(self, key, boot_peer=None):
        ...
        if boot_peer:
            ...
            boot_peer.find_node(key,
                                rpc_id,
                                socket=self.server.socket,
                                peer_id=self.peer.id,
                                peer_info=self.peer.info)
    ...
    def bootstrap(self, bootstrap_nodes=[]):
        ...
        if len(bootstrap_nodes) == 0:
            for bnode in self.buckets.to_list():
                self.iterative_find_nodes(self.peer.id,
                                          boot_peer=Peer(
                                              bnode[0], bnode[1], bnode[2],
                                              bnode[3]))
